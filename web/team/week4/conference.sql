...
CREATE TABLE public.member (
  member_id   SERIAL       constraint  pk_member_1 PRIMARY KEY
  ,member_name VARCHAR(50)  constraint nn_member_2 NOT NULL  constraint uq_member_1 UNIQUE
  ,password  VARCHAR(255) constraint nn_member_3 NOT NULL
);
...
CREATE TABLE public.note (
  ...
  ,member_id    INTEGER  constraint  nn_note_1 NOT NULL
  ...
  ,constraint fk_note_1 foreign key (member_id)    REFERENCES public.member(member_id)
...
INSERT INTO public.note
  ...
  ,note)
  VALUES
 ((SELECT member_id FROM public.member WHERE member_name = 'Tammy')
 ,(SELECT session_id FROM public.session WHERE session = 'Saturday Morning')
 ,(SELECT speaker_id FROM public.speaker WHERE speaker_name = 'Brian K. Taylor')
